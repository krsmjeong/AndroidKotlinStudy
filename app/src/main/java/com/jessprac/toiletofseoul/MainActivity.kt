...
class MainActivity : AppCompatActivity() {
    ...
    // 현재위치를 가져올수 없는 경우 서울시청의 위치로 지도 보여줌
    ...
    fun getMyLocation(): LatLng {
        ...
        if (lastKnownLocation != null) {
            // 위도 경도 객체로 변환
            return LatLng(lastKnownLocation.latitude, lastKnownLocation.latitude)
        } else {
            // 위치를 구하지 못한경우 기본값 반환
            return CITY_HALL
        }
    ...
    fun onMyLocationButtonClick() {
        when {
            hasPermissions() -> googleMap?.moveCamera(
                ...
                applicationContext, "위치 사용 권한 설정에 동의해 주세요", Toast.LENGTH_LONG
            ).show()
        }
    }
    ...
    val API_KEY = "754f6f52636b727336344161464850"
    ...
    fun JSONArray.findByChildProperty(propertyName: String, value: String):JSONObject?{
        // JSONArray 를 순화 하면서 각 JSONObject 의 프로퍼티의 값이 같은지 확인
    ...
    fun addMarkers(toilet: JSONObject){
        ...
        // 아이템 맵에 toilet 객체를 키로 MyItem 객체 저장
